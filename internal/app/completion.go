...
const bashCompletion = `_jumux_complete() {
    ...
    if [[ $COMP_CWORD -eq 1 ]]; then
        COMPREPLY=( $(compgen -W "add remove restart rename rebase attach pr mr list sidebar hook doctor config completion help" -- "$cur") )
        return 0
    fi
    ...
    case "$cmd" in
        ...
        config)
            ...
            ;;
        remove|restart|attach|rename|rebase|pr|mr)
...
_jumux() {
    local -a commands
    commands=(add remove restart rename rebase attach pr mr list sidebar hook doctor config completion help)
...
complete -c jumux -f
complete -c jumux -n "__fish_use_subcommand" -a "add remove restart rename rebase attach pr mr list sidebar hook doctor config completion help"
complete -c jumux -n "__fish_seen_subcommand_from remove restart attach rename rebase pr mr" -a "(__jumux_features)"
